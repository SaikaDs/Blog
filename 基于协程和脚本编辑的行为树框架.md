...
//根据指定的概率
protected bool Check(ref float rate, float delta, float origin)
{
...
//动作协程方法：受击
IEnumerator PerformBeat()
{
    //播放受击动画并等待一个受击硬直时间（硬直时间与动画长度大致相同）
    animator.Rebind();
    animator.Play("hit");
    yield return new WaitForSeconds(self.hitStunTime);
}
...
```